...
const EmployeeProfile = ({ employeeId, onBack }: EmployeeProfileProps) => {
    ...
    const handleDeleteAttendance = async (attendanceId: string) => {
        ...
        try {
            await api.delete(`/reports/attendance/${attendanceId}`);
            // Refresh data
            fetchEmployeeStats();
        } catch (err) {
    ...
    if (!data) return (
        <div className="text-center py-12">
            ...
            <button onClick={onBack} className="mt-4 text-blue-400 hover:underline flex items-center gap-2 mx-auto">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {history.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500 italic">No attendance records found yet.</td>
                                </tr>
                            ) : (
